...
public class WatchSpecification implements Specification<Watch> {

    private final Map<String, String> parameters;

    public WatchSpecification(Map<String, String> parameters) {
        this.parameters = parameters;
    }

    @Override
    public Predicate toPredicate(Root<Watch> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
        Join<Object, Object> joinEmployee = (Join<Object, Object>) root.fetch(Watch.Fields.employee, JoinType.LEFT);

        try {
            List<Predicate> predicates = new ArrayList<>();

            Optional.ofNullable(parameters.get(Watch.Fields.employee + "Id"))
                    .ifPresent(parameter -> {
